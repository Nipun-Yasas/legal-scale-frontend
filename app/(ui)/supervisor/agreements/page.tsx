...
import { HandshakeIcon, ChevronDown, ChevronRight, FileText, Stamp, MessageSquare, XCircle, PenTool } from "lucide-react";
...
export default function SupervisorAgreementsPage() {
  const [agreements, setAgreements] = useState<Agreement[]>([]);
  const [loading, setLoading] = useState(true);
  const [expandedId, setExpandedId] = useState<number | null>(null);

  const [actionModalOpen, setActionModalOpen] = useState(false);
  const [actionType, setActionType] = useState<"APPROVE" | "REJECT" | "SIGN">("APPROVE");
  ...
  const handleActionSubmit = async () => {
    ...
    try {
      if (actionType === "SIGN") {
        await axiosInstance.post(API_PATHS.SUPERVISOR.SIGN(selectedAgreementId.toString()));
      } else {
        const body = {
          reviewStatus: actionType === "APPROVE" ? "APPROVED" : "REJECTED",
          remarks: reviewRemarks || undefined
        };
        await axiosInstance.post(API_PATHS.SUPERVISOR.APPROVAL(selectedAgreementId.toString()), body);
      }
      await fetchAgreements();
      setActionModalOpen(false);
      toast.success(`Agreement ${actionType === "APPROVE" ? "approved" : actionType === "REJECT" ? "rejected" : "signed"} successfully`);
    } catch (error: any) {
      console.error(`Failed to ${actionType.toLowerCase()} agreement`, error);
      const errorMessage = error.response?.data?.message || `Failed to ${actionType.toLowerCase()} agreement`;
    ...
    <div className="flex flex-col gap-8 mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-textPrimary flex items-center gap-2">
          <HandshakeIcon className="h-6 w-6 text-blue-500" /> System Agreements
        </h1>
        <p className="text-sm text-textSecondary mt-1">
          Review and approve pending system agreements
        </p>
      </div>
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                              {a.status === "APPROVED" ? (
                                <button
                                  ...
                                  className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 text-purple-600 hover:bg-purple-100 dark:bg-purple-900/30 dark:text-purple-400 dark:hover:bg-purple-900/50 rounded-lg text-xs font-semibold transition-colors"
                                >
                                ...
                                <>
                                  <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-1.5 bg-green-50 text-green-600 hover:bg-green-100 dark:bg-green-900/30 dark:text-green-400 dark:hover:bg-green-900/50 rounded-lg text-xs font-semibold transition-colors"
                                  >
                                    <Stamp className="h-4 w-4" /> Approve
                                  </button>
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                    ...
                                    className="flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50 rounded-lg text-xs font-semibold transition-colors"
                                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-backgroundSecondary border border-borderPrimary rounded-2xl w-full max-w-md shadow-xl p-6">
            <h2 className="text-lg font-bold text-textPrimary mb-4">
              {actionType === "APPROVE" ? "Approve Agreement" : actionType === "REJECT" ? "Reject Agreement" : "Sign Agreement"}
            </h2>
            {actionType !== "SIGN" ? (
              <div className="mb-4">
                ...
                <textarea
                  ...
                  placeholder={actionType === "APPROVE" ? "Add any remarks regarding the approval..." : "Provide reason for rejection..."}
                  className="w-full p-3 text-sm text-textPrimary bg-backgroundPrimary border border-borderPrimary rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none"
                />
              </div>
            ) : (
              <div className="mb-6 text-sm text-textSecondary">
                Are you sure you want to digitally sign this agreement? This action cannot be undone.
              </div>
            )}
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                disabled={actionLoading || (actionType === "REJECT" && !reviewRemarks.trim())}
                className={cn(
                  "px-4 py-2 rounded-xl text-sm font-medium text-white flex items-center gap-2 transition-colors disabled:opacity-50",
                  actionType === "APPROVE" ? "bg-green-600 hover:bg-green-700" : actionType === "REJECT" ? "bg-red-600 hover:bg-red-700" : "bg-purple-600 hover:bg-purple-700"
                )}
              >
                {actionType === "APPROVE" ? <Stamp className="h-4 w-4" /> : actionType === "REJECT" ? <XCircle className="h-4 w-4" /> : <PenTool className="h-4 w-4" />}
                {actionLoading ? "Processing..." : actionType === "APPROVE" ? "Approve" : actionType === "REJECT" ? "Reject" : "Sign"}
              </button>
            </div>
          </div>
        </div>
