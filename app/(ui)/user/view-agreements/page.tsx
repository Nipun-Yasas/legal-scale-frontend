...
import { HandshakeIcon, ChevronDown, ChevronRight, FileText, UploadCloud, MessageSquare } from "lucide-react";
...
export default function ViewAgreementsPage() {
    ...
    const fetchAgreements = async () => {
        try {
            const response = await axiosInstance.get(API_PATHS.AGREEMENT.GET_OWN);
    ...
    const handleRequestReview = async () => {
        ...
        try {
            const body: any = { reviewStatus: "REVIEW_REQUESTED" };
            if (reviewComment.trim() !== "") {
                body.remarks = reviewComment;
            }
            await axiosInstance.post(API_PATHS.AGREEMENT.REQUEST_REVIEW(selectedAgreementId.toString()), body);
    ...
    const handleUploadRevision = async () => {
        if (!selectedAgreementId || !revisionFile) return;
        setActionLoading(true);
        try {
            ...
            await axiosInstance.post(API_PATHS.AGREEMENT.ADD_REVERSIONS(selectedAgreementId.toString()), formData, {
            ...
            toast.error("Failed to upload document revision");
        } finally {
            setActionLoading(false);
        }
    };
        ...
        <div className="flex flex-col gap-8 mx-auto">
            <div>
                <h1 className="text-2xl font-bold text-textPrimary flex items-center gap-2">
                    <HandshakeIcon className="h-6 w-6 text-blue-500" /> View Agreements
                </h1>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-borderPrimary">
                                    ...
                                    {["ID", "Title", "Type", "Parties", "Value (LKR)", "Duration", "Status", "Submitted"].map(h => (
                            ...
                            <tbody>
                                {[1, 2, 3, 4, 5].map((i) => (
                                    <tr key={i} className="border-b border-borderPrimary last:border-0 h-[68px]">
                                        ...
                                        <td className="px-4 sm:px-6 py-3.5"><div className="w-16 h-4 rounded bg-textSecondary/20 animate-pulse"></div></td>
                            ...
                            {agreements.map(a => (
                                <React.Fragment key={a.id}>
                                    ...
                                    >
                                        ...
                                        <td className="px-4 sm:px-6 py-3.5 font-mono text-xs text-textSecondary">AGR-{a.id.toString().padStart(3, '0')}</td>
                                                    ...
                                                    <div className="flex items-center justify-between mb-4 mt-2">
                                                        ...
                                                        <div className="flex flex-wrap items-center gap-3">
                                                            ...
                                                            <button
                                                                ...
                                                                className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-900/50 rounded-lg text-xs font-semibold transition-colors"
                                                            >
                                                                <MessageSquare className="h-4 w-4" /> Request Review
                                                            </button>
                                                        </div>
                                                    </div>
            ...
            {requestReviewModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="bg-backgroundSecondary border border-borderPrimary rounded-2xl w-full max-w-md shadow-xl p-6">
                        <h2 className="text-lg font-bold text-textPrimary mb-4">Request Review</h2>
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-textPrimary mb-1">Comment</label>
                            <textarea
                                rows={4}
                                value={reviewComment}
            ...
            {revisionModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="bg-backgroundSecondary border border-borderPrimary rounded-2xl w-full max-w-md shadow-xl p-6">
                        <h2 className="text-lg font-bold text-textPrimary mb-4">Upload New Document</h2>
                        <div className="flex flex-col gap-4">
                            <div>
                                <label className="block text-sm font-medium text-textPrimary mb-1">Document File</label>
                                <input
                                    type="file"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-textPrimary mb-1">Revision Notes</label>
                                <textarea
                                    rows={3}
                                    value={revisionNotes}
